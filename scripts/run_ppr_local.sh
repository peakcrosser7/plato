...
MAIN="$ROOT_DIR/bazel-bin/example/personalized_pagerank" # process name
WNUM=4
WCORES=4

INPUT=${INPUT:="$ROOT_DIR/data/graph/v100_e2150_ua_c3.csv"}
OUTPUT=${OUTPUT:="/tmp/personalized_pagerank"}
...
PARAMS+=" --iterations ${ITERATIONS} --eps ${EPS} --damping ${DAMPING} --src ${SRC}"
